...
function onChangeText() {
  ...
  function newLine(cLine) {
    ...
    //  console.log(typeof cLine);

    if (cLine) {
      var column = document.createElement("div");
      column.classList.add("csv-diff-column");
      column.appendChild(document.createTextNode("Row  " + cLine));
  ...
  for (var i = 0; i <= maxColumn; i++) {
    ...
    column.classList.add("csv-diff-column");
    if (i > 0) {
      column.appendChild(document.createTextNode("Column " + i));
    }
    line.appendChild(column);
  }
  textByLine1.forEach(function (line1, indexLine) {
    ...
    if (textByLine2.length > indexLine) {
      ...
      for (
        var i = Math.max(textByColumn1.length, textByColumn2.length);
        i < maxColumn;
        i++
      ) {
        diffText += separator;
...
document.getElementById("file-input-text").addEventListener(
  "change",
  function (e) {
...
document.getElementById("file-input-text-2").addEventListener(
  "change",
  function (e) {
    readSingleFile(e, function (data) {
...
function get(url, callback, callbackError, options) {
  ...
  xhr.onreadystatechange = function () {
    ...
    callback(
      options && options.responseType && options.responseType == "blob"
        ? this.response
        : this.responseText,
      this
    );
...
window.addEventListener(
  "load",
  function () {
    myCodeMirrorText1 = CodeMirror.fromTextArea(
      document.getElementById("editor-container-text"),
      { lineNumbers: true, viewportMargin: Infinity }
    );
    myCodeMirrorText2 = CodeMirror.fromTextArea(
      document.getElementById("editor-container-text-2"),
      { lineNumbers: true, viewportMargin: Infinity }
    );
    ...
    document.getElementById("download-diff").addEventListener(
      "click",
      function (e) {
        exportDiff({
          data: diffText,
    ...
    dialogLoadFromURL = new A11yDialog(
      document.getElementById("dialog-load-from-url")
    );
